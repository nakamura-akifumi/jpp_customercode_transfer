...
describe JppCustomercodeTransfer::ZipCodeList do
  ...
  before(:all) do
    @importfile_path = "#{::Rails.root}/../../spec/csv"
  end
  ...
  context "generate barcode" do
    ...
    it "shoud raise exception (1)" do
      proc {
        JppCustomercodeTransfer::ZipCodeList.generate_japanpost_customer_code(nil , nil) 
      }.should raise_error(ArgumentError, "zip_code or address is empty.")
    end

    it "shoud raise exception (2)" do
      proc {
        JppCustomercodeTransfer::ZipCodeList.generate_japanpost_customer_code("3340021" , nil) 
      }.should raise_error(ArgumentError, "zip_code or address is empty.")
    end

    it "shoud raise exception (3)" do
      proc {
        JppCustomercodeTransfer::ZipCodeList.generate_japanpost_customer_code("113-0021" , "東京都文京区本駒込6-1-21") 
    ...
    it "shoud return customer code (1)" do
      ...
      ans = JppCustomercodeTransfer::ZipCodeList.generate_japanpost_customer_code(h[:z], h[:a]) 
      ans.should eq h[:ans]
    end
    ...
    it "shoud return customer code (15)" do
      ...
      proc {
        JppCustomercodeTransfer::ZipCodeList.generate_japanpost_customer_code(h[:z], h[:a]) 
      }.should raise_error(ArgumentError, "zip_code is invalid. (no record)")
    end
